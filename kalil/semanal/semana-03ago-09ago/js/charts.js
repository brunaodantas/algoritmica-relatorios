// ===== CHARTS — ALEXANDRE KALIL | PRÉ-CAMPANHA 2026 (03/08 a 09/08) =====
...
if (ctxInv) {
  new Chart(ctxInv, {
    type: 'doughnut',
    data: {
      labels: ['Estado de Minas Gerais', 'Público Cleitinho', 'RMBH', 'Triângulo Mineiro'],
      datasets: [{ data: [343.03, 260.92, 178.18, 102.03], backgroundColor: [C.accent, C.g300, C.purple, C.g700], borderColor: '#fff', borderWidth: 4, hoverOffset: 8 }]
    },
    options: {
      maintainAspectRatio: false, cutout: '62%',
      plugins: {
        legend: { position: 'bottom' },
        tooltip: { ...TIP, callbacks: { label: c => ' R$ ' + c.raw.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) + ' (' + (c.raw / 884.16 * 100).toFixed(1).replace('.', ',') + '%)' } }
      }
    }
  });
}
...
if (ctxPracas) {
  new Chart(ctxPracas, {
    type: 'bar',
    data: {
      labels: ['Estado de Minas Gerais', 'Público Cleitinho', 'Triângulo Mineiro', 'RMBH'],
      datasets: [{ data: [67224, 58077, 40740, 34474], backgroundColor: [C.accent, C.g300, C.g700, C.purple], borderRadius: 6, barThickness: 34 }]
    },
    options: {
...
// ── Barras — Custo por ThruPlay por Segmentação ──
const ctxCusto = document.getElementById('chartCusto');
if (ctxCusto) {
  new Chart(ctxCusto, {
    type: 'bar',
    data: {
      labels: ['Público Cleitinho', 'Estado de Minas Gerais', 'RMBH'],
      datasets: [{ data: [0.0181, 0.0269, 0.0334], backgroundColor: [C.g300, C.accent, C.purple], borderRadius: 6, barThickness: 40 }]
    },
    options: {
      maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { ...TIP, callbacks: { label: c => ' R$ ' + c.raw.toFixed(4).replace('.', ',') + ' por ThruPlay' } } },
      scales: { y: { grid: { color: 'rgba(0,0,0,.06)' }, ticks: { callback: v => 'R$ ' + v.toFixed(3).replace('.', ',') } }, x: { grid: { display: false } } }
    }
  });
}
...
if (ctxAnuncios) {
  new Chart(ctxAnuncios, {
    type: 'bar',
    data: {
      labels: ['Eu não faço promessa V2', 'Kalil Faz · carrossel', 'VT Mulheres', 'Aftermovie Convenção', 'Eu não faço promessa'],
      datasets: [{ data: [58077, 40740, 40078, 34474, 27146], backgroundColor: C.accent, borderRadius: 6, barThickness: 30 }]
    },
    options: {
      indexAxis: 'y', maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { ...TIP, callbacks: { label: c => ' ' + c.raw.toLocaleString('pt-BR') + ' impressões' } }, dataLabels: { enabled: true } },
...
if (ctxIdade) {
  new Chart(ctxIdade, {
    ...
    data: {
      ...
      datasets: [{ data: [3.2, 11.7, 17.4, 32.2, 26.4, 9.1], backgroundColor: C.accent, borderRadius: 6, barThickness: 36 }]
    },
    options: {
      maintainAspectRatio: false,
...
if (ctxGenero) {
  new Chart(ctxGenero, {
    ...
    data: {
      ...
      datasets: [{ data: [64.6, 35.3, 0.1], backgroundColor: [C.g900, C.accent, C.g300], borderColor: '#fff', borderWidth: 4, hoverOffset: 8 }]
    },
    options: {
      maintainAspectRatio: false, cutout: '62%',
...
if (ctxPos) {
  new Chart(ctxPos, {
    ...
    data: {
      ...
      datasets: [{ data: [148224, 25022, 20480, 6734], backgroundColor: C.purple, borderRadius: 6, barThickness: 30 }]
    },
    options: {
      indexAxis: 'y', maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { ...TIP, callbacks: { label: c => ' ' + c.raw.toLocaleString('pt-BR') + ' impressões' } }, dataLabels: { enabled: true } },
